...
class AccountUpdateForm(FlaskForm):
    ...
    image = FileField('Εικόνα Προφίλ', validators=[Optional(strip_whitespace=True),
                        ...
                        maxImageSize(max_size=2)]) ## Αρχείο Εικόνας, με επιτρεπόμενους τύπους εικόνων τα 'jpg', 'jpeg', 'png', και μέγιστο μέγεθος αρχείου εικόνας τα 2 MBytes, ΜΗ υποχρεωτικό πεδίο
...
class NewMovieForm(FlaskForm):
    ...
    plot = TextAreaField(label="Υπόθεση Ταινίας",
                            validators=[DataRequired(message="Αυτό το πεδίο δε μπορεί να είναι κενό."), 
                                       Length(min=5, message="Το κείμενο της υπόθεσης πρέπει να έχει τουλάχιστον 5 χαρακτήρες.")])
    ...
    image = FileField('Εικόνα Ταινίας', validators=[Optional(strip_whitespace=True),
                        FileAllowed(['jpg', 'jpeg', 'png'], 'Επιτρέπονται μόνο αρχεία τύπου jpg, jpeg, png.'), maxImageSize(max_size=2)]) 
    ...
    release_year = IntegerField(label="Έτος Πρώτης Προβολής Ταινίας",
                                    validators=[DataRequired(message="Αυτό το πεδίο δε μπορεί να είναι κενό."), 
                                        NumberRange(min=1888, max=current_year, message=f"Αυτό το πεδίο παίρνει ακέραιες τιμές από το 1888 έως το {current_year}.")]) 
    ...
    rating = IntegerField(label="Βαθμολογία Ταινίας",  validators=[DataRequired(message="Αυτό το πεδίο δε μπορεί να είναι κενό."), 
                            NumberRange(min=1, max=100, message="Αυτό το πεδίο παίρνει ακέραιες τιμές από το 1 έως το 100.")]) 
